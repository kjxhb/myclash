...
# 删除clash.yaml的原节点
sed -i '/github.com\/Ruk1ng001/d' oldclash.yaml

# 在clash.yaml中查找第一个proxies，并在其下一行插入1.yaml中的节点（添加节点）
line1=$(grep -n 'proxies' oldclash.yaml | sed -n '1s/:.*//p')
sed -i "${line1}r 1.yaml" oldclash.yaml

# 在clash.yaml中查找第二个proxies，并在其下一行插入2.yaml的节点名称（添加“自动选择”分组的节点名称）
line2=$(grep -n 'proxies' oldclash.yaml | sed -n '2s/:.*//p')
sed -i "${line2}r 2.yaml" oldclash.yaml

# 在clash.yaml中查找第二个'自动选择'，并在其下一行插入2.yaml的节点名称（添加“节点选择”分组的节点名称）
line3=$(grep -n '自动选择' oldclash.yaml | sed -n '2s/:.*//p')
sed -i "${line3}r 2.yaml" oldclash.yaml
